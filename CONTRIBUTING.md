...
## 3.1 Integração Contínua (CI/CD) e Build Automático

Todo Pull Request aberto para a branch `main` passará por uma verificação automática no **GitHub Actions (CI)**. O pipeline irá compilar o projeto utilizando **Maven (JDK 21)**.

- Se o código compilar com sucesso, será exibido um **check verde** ✅ no Pull Request.
- Se houver erros de compilação (como ponto e vírgula ausente, classe faltando ou dependência incorreta), será exibido um **X vermelho** ❌.

**REGRA:** Nenhum Pull Request será aprovado ou mesclado enquanto o CI estiver falhando. Caso o status fique vermelho, verifique os logs no GitHub Actions, corrija o código localmente, faça um novo commit e envie (`push`) novamente para reexecutar o CI.
